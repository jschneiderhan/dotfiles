...
export default Controller.extend({
  ...
  _findOrCreateImplementation() {
    return this.get('session.user').then( user => {
      if (user) {
        return this.get('store').query('saas-implementation', { filter: { user: { wbid_key: this.get('session.user.wbidUserKey')} } } ).then((saasImplementation) => {
        })
      }
    }).then((saasImplementation) => {
      if (!saasImplementation) {
        this._createImplementation();
      }
    })
          if (saasImplementations.get('length') == 0) {
            return this._createImplementation();
          } else {
            return saasImplementations.get('firstObject')
          }
    })
    return this.get('session').load().then(() => {
    }).then((saasImplementations) => {
      debugger
    })
  },
