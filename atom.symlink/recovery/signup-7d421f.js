...
export default Controller.extend({
  ...
  _createImplementation() {
    this.get('store').query('saasImplementation', { filter: { user_id: 9} }).then(function(saasImplementation) {
      if (saasImplementation.get('length') == 0){
        return this.get('stripe').createToken(this.get('stripeElement')).then(({ token }) => {
          return this.get('store').createRecord('saas-implementation', { companyName: this.get('companyName'), billingPlan: this.get('plan'), timeZone: moment.tz.guess(), cardToken: token.id }).save().then((saasImplementation) => {
