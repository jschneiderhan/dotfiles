      ...
      attributes_table_for saas_implementation do
        ...
        row "Purchaser Information" do
          ...
          metrics = [
                ...
                SELECT DISTINCT
                  ...
                  FROM c.wbid_sponsor_profiles sp
                    INNER JOIN c.wbid_product_registrations pr ON sp.user_wbid_key = pr.user_wbid_key
                      ...
                      AND service = 'dc'
                  WHERE sp.segment_key::uuid = 'a885486c-6881-4239-b259-4297833053b4'
                  AND pr.user_wbid_key='001fe8osx0r5s';").,
              signed_in: "Yes",
              activity: [
                {
            ...
            },
            {
              ...
              activity: [
                {
                  name: "Connected a Device",
                  value: "No"
                },
                {
                  name: "Followed Another User",
                  value: "Yes"
                }
              ]
            }
          ...
          table_for metrics do
            ...
            column "Signed in", :signed_in
            column "User Has" do |metric|
              text_node metric[:activity].collect {|a| "#{a[:name]}: #{a[:value]}"}.join('<br/>').html_safe
            end
          end
        end

        row "Non-purchaser Information" do
          metrics = [
            {
              name: "Daily Challenge",
              enrolled: "11",
          ...
          table_for metrics do
            ...
            column "Enrolled", :enrolled
            column "Signed in", :signed_in
            column "# Users that Have" do |metric|
              text_node metric[:activity].collect {|a| "#{a[:name]}: #{a[:value]}"}.join('<br/>').html_safe
            end
          end
        end
        ...
        row "User Information" do
          metrics = [
            ...
            { name: "Well-Being Tracker", users_metric: Metrics::Wbt::Users, engaged_participants_metric: Metrics::Wbt::EngagedParticipants },
          ]
          table_for metrics do
            column "Product", :name
            column "Total" do |m|
