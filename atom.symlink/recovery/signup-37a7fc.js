...
export default Controller.extend({
  session: service(),
  saasImplementationManager: service(),
  router: service(),
  ...
  actions: {
    ...
    login() {
      ...
      return this.get('ajax').request(`${ENV.APP.accountUrl}/api/user/session`, {
        ...
        return this.get('session').load().catch(resolve).then(() => {
          return this._findOrCreateImplementation().then(() => {
            $('.ui.LoginModal.modal').modal('hide');

            return this._flashSuccessAndTransition();

        // We want to load an existing snap user if one exists.
        return this.get('session').load().catch(() => {
          // This is going to fail with a 401 if a snap user doesn't
          // exist yet, which is fine.
        }).then(() => {
          // There's no point in looking for an implementation if
          // a user doesn't exist
          if (this.get('session.user')) {
            return this._findExistingImplementation();
          }
        }).then((existingImplementation) => {
          if (!existingImplementation) {
            return this._createImplementation().catch((e) => {
  ...
  _findOrCreateImplementation() {
    return this.get('saasImplementationManager.saasImplementation').then((implementation) => {
      if (implementation) {
        return implementation;
      } else {
        return this._createImplementation();
      }
    });
  },

  _findExistingImplementation() {
    return this.get('saasImplementationManager.saasImplementation');
  },
