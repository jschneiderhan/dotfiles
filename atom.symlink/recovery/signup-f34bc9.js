...
export default Controller.extend({
  ...
  _createImplementation() {
    return this.get('stripe').createToken(this.get('stripeElement')).then(({ token }) => {
      return this.get('store').createRecord('saas-implementation', { companyName: this.get('companyName'), billingPlan: this.get('plan'), timeZone: moment.tz.guess(), cardToken: token.id });
    }).then((saasImplementation) => {
      return saasImplementation.save;
    }).then((saasImplementation) => {
      return saasImplementation.setProperties({ cardToken: undefined, timeZone: undefined });
    }).then(() =>{
