    ...
    panel "Participation Information" do
      attributes_table_for saas_implementation do
        ...
        row "User Information" do
          metrics = [
            { name: "Daily Challenge", metric: Metrics::Dc::Users },
            { name: "QuitNet", metric: Metrics::Qn::Users },
            { name: "Walkadoo", metric: Metrics::Wd::Users },
            { name: "Well-Being ID", metric: Metrics::Pl::Users },
            { name: "Well-Being Tracker", metric: Metrics::Wbt::Users },
          ]
          table_for metrics do
            column "Product", :name
            column "Total Users" do |m|
              value = m[:metric].new([saas_implementation.segment], saas_implementation.starts_at).data_points.to_h["totalUsers"]
              status = case value
              when 0..1
                "red"
              when 2..5
        ...
        row "Daily Challenge - # of users who have completed a challenge" do
          Sequel.connect(Rails.application.secrets.dwh_url)
                .fetch("select count(distinct(user_wbid_key)) from e.e_completed_challenge where user_segment_key=?", )
        end
      end
    end
